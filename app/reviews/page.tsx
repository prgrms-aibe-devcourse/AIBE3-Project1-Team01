...
const mockReviews = [
  {
    ...
    content: '한라산 등반부터 해변까지, 제주도의 모든 매력을 만끽했습니다. 특히 성산일출봉에서 본 일출이 정말 감동적이었어요!',
    author: '여행러버',
    region: '제주도',
    rating: 5,
    images: [
  ...
  },
  {
    ...
    content: '알록달록한 집들과 골목길 곳곳의 예술 작품들이 너무 아름다웠어요. 인스타그램 사진 찍기에도 최고!',
    author: '아트홀릭',
    region: '부산',
    rating: 4,
    images: [
  ...
  },
  {
    ...
    content: '경복궁, 창덕궁, 덕수궁까지 하루에 돌아본 후기입니다. 한복 체험도 함께해서 더욱 의미있었어요.',
    author: '역사탐험가',
    region: '서울',
    rating: 5,
    images: [
  ...
  },
  {
    id: '4',
    title: '강원도 설악산 단풍 여행',
    content: '가을 단풍이 절정일 때 다녀온 설악산! 케이블카를 타고 올라간 전망이 정말 장관이었습니다.',
    author: '산행매니아',
    region: '강원도',
    rating: 5,
    images: [
...
export default function ReviewsPage() {
  ...
  const handleFilterChange = (filters: any) => {
    ...
    let filtered = mockReviews.filter(review => {
      const regionMatch = filters.region === 'all' || review.region === filters.region;
      const categoryMatch = filters.category === 'all' || review.category === filters.category;
      const ratingMatch = filters.rating === 'all' || review.rating >= parseInt(filters.rating);
      
      return regionMatch && categoryMatch && ratingMatch;
    });
      ...
      <header className="bg-white/80 backdrop-blur-sm border-b border-pink-100 sticky top-0 z-40">
        <div className="container mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <Link href="/" className="text-pink-500 hover:text-pink-600 cursor-pointer">
